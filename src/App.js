...
class ReactSelection extends Component {
  ...
  // dummy data. Real data should come from props
  ...
  render() {
    ...
    if(this.state.selectedItems.length > 0) {
      selected = this.state.selectedItems.map(
          (item, index) => <option className="move-backward" onClick={this.onClickHandler} onDoubleClick={this.onDoubleClickHandler} key={index} id={index} value={item.value}>
    ...
    if(options.length > 0) {
      ...
      options = this.state.filteredCategories.map((field, index) =>
          <option className="move-forward" onClick={this.onClickHandler} onDoubleClick={this.onDoubleClickHandler} key={index} id={index} value={field.value}>{field.prettyName}</option>
      );
    }
